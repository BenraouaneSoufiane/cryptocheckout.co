const hostname = "api.cryptocheckout.co";
const port = 8080;
          ...
          (async () => {
            ...
            try {
              ...
              const secret = bs58.encode(
                Buffer.from(JSON.stringify(ok), "utf8")
              );
              res.end(
                '{"address":"' +
                address +
...
(async()=>{
    const connection = setInterval(function(){
        try{
            ...
            const path = 'C:/Apache24/htdocs/cryptocheckout.co/api/w/'+JSON.stringify(Date.now());
            let walletFull = await monerojs.createWalletFull({
              //path: path,
              password: "getpaidcrypto",
              networkType: "mainnet",
  ...
  if (queryObject.transaction == "btc") {
      ...
      (r) => {
        ...
        r.on("end", () => {
          ...
          if (Number(result["balance"]) >= Number(queryObject.amount)) {
            ...
            var privateKey = new Mnemonic(
              Buffer.from(queryObject.pk, "hex").toString()
            )
              .toHDPrivateKey()
              .derive("m/84'/0'/0'/0/0").privateKey;

            for (i = 0; i <= result.txs.length - 1; i++) {
              for (j = 0; j <= result.txs[i]["outputs"].length - 1; j++) {
                if (
                  result.txs[i]["outputs"][j]["addresses"][0] == queryObject.src
                ) {
                  for (o = 0; o <= result.txs[i]["inputs"].length - 1; o++) {
                    if (o == result.txs[i]["inputs"].length - 1) {
                      utxos.push({
                        txId: result.txs[i]["hash"],
                        outputIndex: result.txs[i].inputs[o]["output_index"],
                        address: result.txs[i].inputs[o]["addresses"][0],
                        script: result.txs[i]["outputs"][j]["script"],
                        satoshis: result.txs[i]["outputs"][j]["value"],
                      });
                      ...
                      if (Number(queryObject.amount) <= amount) {
                        ...
                        transaction
                          ...
                          .to(
                            queryObject.dst,
                            Number(queryObject.amount) - 1500
                          )
                          .change(queryObject.src)
                          .sign(privateKey)
                          .serialize();
    ...
    const deploy = async () => {
      try {
        ...
        try {
          const receipt = await web3eth.eth.sendSignedTransaction(
            createTransaction.rawTransaction
          );
          if (receipt.transactionHash !== "") {
            res.end(
              '{"result":"success","tx":' + receipt.transactionHash + '"}'
            );
          } else {
    ...
    const Web3eth = require("web3"),
      webprovider = new Web3.providers.HttpProvider(
    ...
    deploysol = async () => {
      ...
      const keypair = web3.Keypair.fromSeed(
        derivePath(`m/44'/501'/0'`, seed.toString("hex")).key
      );
      ...
      try {
        const tr = await web3.sendAndConfirmRawTransaction(
          connection,
          rawTransaction
        );
    ...
    const pk = CardanoWasm.Bip32PrivateKey.from_bip39_entropy(
          ...
          if (total <= Number(queryObject.amount)) {
            txBuilder.add_key_input(
              ...
              CardanoWasm.Value.new(
                CardanoWasm.BigNum.from_str(rutxos[i].amount[0].quantity)
              )
            );
          ...
          if (i == rutxos.length - 1) {
            ...
            txBuilder.add_output(
              CardanoWasm.TransactionOutput.new(
                ...
                CardanoWasm.Value.new(
                  CardanoWasm.BigNum.from_str(queryObject.amount)
                )
              )
            );
            rstr = "";
    ...
    const InMemorySigner = require("@taquito/signer"),
      TezosToolkit = require("@taquito/taquito");
    (async () => {
      const Tezos = new TezosToolkit.TezosToolkit(
    ...
    const src = JSON.parse(
      Buffer.from(bs58.decode(queryObject.src)).toString()
    ).path;
