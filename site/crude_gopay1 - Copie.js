                            ...
                            if(typeof amount.btc !== 'undefined' && typeof address.btc !== 'undefined' ) {
                                $.get(
                                    'https://crypto.hirak.site/?address='+address.btc+'&amount='+amount.btc,function (response) {
                                        $('#'+div+' #cntnr0').append('<div id="cntnr" style="clear:both"><div type="'+j+'" id="'+j+'"><span class="cryptoicons" type="'+j+'" id="'+j+'icon" style="float:left;clear:both;width: 32px;height: 32px;display: none;background-image: url(\'https://crypto.hirak.site/'+j+'.png\');float:left"></span></div></div></div>');
                                        $('#'+div+' #cntnr0 #btc').append('<p class="guide0" type="'+j+'" style="display:none;text-align: center;position:relative;left:10px;bottom:10px;float:left">'+messages.guide0+' '+amount.j+j.toUpperCase()+'</p><img id="qr" style="display:none;margin-right:auto;margin-left:auto;width:60%" src="https://crypto.hirak.site/btc/'+address.btc+amount.btc+'.png" ><p id="address" type="'+j+'" style="display:none;text-align: center">'+address.btc+'</p>');
                                        type = 'btc';
                                ...
                                $.get(
                                    'https://rates.hirak.site/rate.php?from=usd&to=btc&token=6fd8404714f243391d3f125910b4338a',function (btcresponse) {
                                        $.get(
                                            'https://crypto.hirak.site/?address='+address.btc+'&amount='+parseFloat(btcresponse.rate).toFixed(6).toString(),function (response1) {
                                                if(btcresponse.to == 'BTC' ) {
                                                    $('#'+div+' #cntnr0').append('<div id="cntnr" style="clear:both" ><div type="'+btcresponse.to.toLowerCase()+'" id="'+btcresponse.to.toLowerCase()+'"><span class="cryptoicons" type="'+btcresponse.to.toLowerCase()+'" id="'+btcresponse.to.toLowerCase()+'icon" style="float:left;clear:both;width: 32px;height: 32px;display: none;background-image: url(\'https://crypto.hirak.site/'+btcresponse.to.toLowerCase()+'.png\');float:left"></span></div></div></div>');
                                                    $('#'+div+' #cntnr0 #'+btcresponse.to.toLowerCase()).append('<p class="guide0" type="'+btcresponse.to.toLowerCase()+'" style="display:none;text-align: center;position:relative;left:10px;bottom:10px;float:left">'+messages.guide0+' '+(parseFloat(amount.usd)*parseFloat(btcresponse.rate)).toFixed(6).toString()+btcresponse.to+'</p><img id="qr" style="display:none;margin-right:auto;margin-left:auto;width:60%" src="https://crypto.hirak.site/btc/'+address.btc+parseFloat(btcresponse.rate).toFixed(6).toString()+'.png" ><p id="address" type="'+btcresponse.to.toLowerCase()+'" style="display:none;text-align: center">'+address.btc+'</p>');                                        
                                                    amount[j.toLowerCase()]=(parseFloat(amount.usd)*parseFloat(btcresponse.rate)).toFixed(6).toString();guide0(div);
                                                    type='btc';
                                                    guide0(div,type,amount);
                                                }
                                            }
                ...
                $('#nextstep').on(
                    'click',function (e) {
                        if($('#'+div+' #source').val() !=='' && $('#'+div+' #source').val().length == 6 ) {
                            ...
                            $('#'+div+' #cntnr0 #'+type).prepend('<span id="'+type+'icon" style="width: 32px;height: 32px;display: block;background-image: url(\'https://crypto.hirak.site/'+type+'.png\');margin-right:auto;margin-left:auto"></span>');
                            $.get(
                                'https://crypto.hirak.site/?submit='+$('#'+div+' #source').val()+'&amount='+amount[type.toLowerCase()]+'&curr='+type,function (response) {
                            ...
                            $('#'+div+' button#ivepaid').on(
                                'click',function (e) {
                                    ...
                                    //hash='f3b248e4bcbb80c169a6dd67d140cf1c';
                                    $.get(
                    ...
                    'keypress',function (e) {                                
                        if(e.which == 13 ) {
                            if($('#'+div+' #source').val() !=='' && $(this).val().length == 6 ) {
                                ...
                                $.get(
                                    'https://crypto.hirak.site/?submit='+$(this).val()+'&amount='+amount[type.toLowerCase()]+'&curr='+type,function (response) {
                                        hash = response.result;
                                    }
                                );
                                $('#'+div+' .guide0,#'+div+' #guide1,#'+div+' #source').remove();
