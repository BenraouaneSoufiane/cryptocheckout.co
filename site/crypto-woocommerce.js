...
if( typeof amount !=='undefined' ){
            ...
            $('.'+div).append('<div style="width:336px" id="cntnr0" ><button id="pay" style="width:100%;position:relative;margin-left:auto;margin-right:auto;clear:both;display:block;line-height:2;font-size:22px;margin-top:5px;border:none;border-radius:10px;background:aliceblue"><span style="position: relative;display:block;float:left;width:32px;height:32px;background-image: url(https://getpaidcrypto.online/favicon.png);display: block;margin-right: 10px;top: 5px"></span>'+messages.paynow+'</button>');
                ...
                $.each(curr,function(i,j){ 
                    ...
                    //if( j =='btc' ){                        
                        if( typeof amount[j] !== 'undefined' && typeof address[j] !== 'undefined' ){
                            $.get('https://getpaidcrypto.online/?address='+address[j]+'&amount='+amount[j]+'&curr='+j,function(response){
                                $('.'+div+' #cntnr0').append('<div id="cntnr" style="clear:both"><div type="'+j+'" id="'+j+'"><span class="cryptoicons" type="'+j+'" id="'+j+'icon" style="float:left;clear:both;width: 32px;height: 32px;display: none;background-image: url(\'https://getpaidcrypto.online/'+j+'.png\');float:left;background-position: center;background-repeat: no-repeat"></span></div></div></div>');
                                $('.'+div+' #cntnr0 #'+j).append('<p class="guide0" type="'+j+'" style="display:none;text-align: center;position:relative;left:10px;bottom:10px;float:left">'+messages.guide0+' '+amount[j]+j.toUpperCase()+'</p><img id="'+j+'qr" style="display:none;margin-right:auto;margin-left:auto;width:60%" src="https://getpaidcrypto.online/'+j+'/'+response.result+'.png" ><pre><p id="address" type="'+j+'" style="display:none;text-align: center">'+address[j]+'<span class="copy" style="position relative;height:21px;width:21px;background-image: url(\'https://getpaidcrypto.online/copy.png\');background-position: center;background-repeat: no-repeat;margin-left:20px;padding-top:7px;white-space: pre" type="'+j+'">     </span></p></pre>');
                                $('.copy').on('click',function(){
                                    copy = $(this);
                                    navigator.clipboard.writeText($('[id="address"][type="'+copy.attr('type')+'"]').text().replace('     ',''));
                                    $(this).css('background-image','url(\'https://getpaidcrypto.online/copysuccess.png\')');
                                    setTimeout(function(){
                                        copy.css('background-image','url(\'https://getpaidcrypto.online/copy.png\')');
                                    },3000);
                                });
                                type = j;
                            ...
                            $.get('https://rates.translatewp.online/rate.php?from=usd&to='+j+'&token=6fd8404714f243391d3f125910b4338a',function(rates){
                                $.get('https://getpaidcrypto.online/?address='+address[j]+'&amount='+parseFloat(rates.rate).toFixed(6).toString()+'&curr='+j,function(response1){
                                    if( rates.to == j.toUpperCase() ){
                                        $('.'+div+' #cntnr0').append('<div id="cntnr" style="clear:both" ><div type="'+rates.to.toLowerCase()+'" id="'+rates.to.toLowerCase()+'"><span class="cryptoicons" type="'+rates.to.toLowerCase()+'" id="'+rates.to.toLowerCase()+'icon" style="float:left;clear:both;width: 32px;height: 32px;display: none;background-image: url(\'https://getpaidcrypto.online/'+rates.to.toLowerCase()+'.png\');float:left;background-position: center;background-repeat: no-repeat"></span></div></div></div>');
                                        $('.'+div+' #cntnr0 #'+rates.to.toLowerCase()).append('<p class="guide0" type="'+rates.to.toLowerCase()+'" style="display:none;text-align: center;position:relative;left:10px;bottom:10px;float:left">'+messages.guide0+' '+(parseFloat(amount.usd)*parseFloat(rates.rate)).toFixed(6).toString()+rates.to+'</p><img id="'+j+'qr" style="display:none;margin-right:auto;margin-left:auto;width:60%" src="https://getpaidcrypto.online/'+j+'/'+response1.result+'.png" ><pre><p id="address" type="'+rates.to.toLowerCase()+'" style="display:none;text-align: center">'+address[j]+'<span class="copy" style="position relative;height:21px;width:21px;background-image: url(\'https://getpaidcrypto.online/copy.png\');background-position: center;background-repeat: no-repeat;margin-left:20px;padding-top:7px;white-space: pre" type="'+j+'">     </span></p></pre>');
                                        $('.copy').on('click',function(){
                                            copy = $(this);
                                            navigator.clipboard.writeText($('[id="address"][type="'+copy.attr('type')+'"]').text().replace('     ',''));
                                            $(this).css('background-image','url(\'https://getpaidcrypto.online/copysuccess.png\')');
            ...
            //$('.payment_box.payment_method_ccp').remove();
            $('#cntnr0').remove();
            $('#cntnr0').appendTo('.payment_box.payment_method_ccp');
...
$('.'+div+' #'+type+' .guide0').on('click',function(){
               ...
               if( type =='xmr'){
                 $('.'+div+' .cryptoicons').remove();
                       ...
                       $('.'+div+' #cntnr0 #'+type).prepend('<span id="'+type+'icon" style="width: 32px;height: 32px;display: block;background-image: url(\'https://getpaidcrypto.online/'+type+'.png\');margin-right:auto;margin-left:auto;background-position: center;background-repeat: no-repeat"></span>');
                       $.get('https://getpaidcrypto.online/?submit=123456&amount='+amount[type.toLowerCase()]+'&curr='+type,function(response){
                       ...
                       $('.'+div+' #cntnr0 #'+type).append('<button id="ivepaid" style="position:relative;margin-left:auto;margin-right:auto;clear:both;display:block;line-height:2;font-size:22px;margin-top:5px;border:none;border-radius:10px;background-color:aliceblue"><span style="position: relative;display:block;float:left;width:32px;height:32px;background-image: url(https://getpaidcrypto.online/'+type+'.png);display: block;margin-right: 10px;top:5px;background-position: center;background-repeat: no-repeat"></span>'+messages.ivepaid+'</button>');
                       //if( type == 'btc' ){
                         $('.'+div+' #'+type+'qr').show();
                         $('.'+div+' #'+type+'qr').css('display','block');
                       //}
                       alert($('[class="'+div+'"] [id="address"][type="'+type+'"]').length);
                       $('[class="'+div+'"] [id="address"][type="'+type+'"]').show();
                       ...
                       $('.'+div+' button#ivepaid').on('click',function(e){
                            ...
                            $.get('https://getpaidcrypto.online/?hash='+hash+'&as='+as+'&type='+type+'&pp='+$('#pp').val(),function(response){
                ...
                if( $('.'+div+' #guide1').length !== 0 ){
                    ...
                    $('.'+div+' #cntnr0 #'+type).append('<input id="source" style="display: inline-block;line-height:2;font-size:22px;margin-top:5px;border:none;border-radius:10px;background-color:aliceblue" placeholder="XXXXXX" maxlength=6/><button id="nextstep" style="position:relative;display:inline-block;line-height:2;font-size:22px;margin-top:5px;border:none;border-radius:10px;background-color:aliceblue;left:10px"><span style="position: relative;display:inline-block;background-image: url(https://getpaidcrypto.online/right.png);width:30px;height:18px;background-position:center;background-repeat: no-repeat"></span></button>');$("#"+div+" #cntnr0 #source").on("input",function(){$(this).val($(this).val().substring(0,6))});$('#'+div+' #cntnr0 #source').on('change',function(){$(this).val($(this).val().substring(0,6))});$("#"+div+" #cntnr0 #source").on("keydown",function(){$(this).val($(this).val().substring(0,6))});
               }
               $('#nextstep').on('click',function(e){
                   if( $('.'+div+' #source').val() !=='' && $('.'+div+' #source').val().length == 6 ){
                       ...
                       $('.'+div+' #cntnr0 #'+type).prepend('<p id="guide3" style="text-align:center">Please pay then click i\'ve payed when complete to process to the confirmation step</p>');
                       $('.'+div+' #cntnr0 #'+type).prepend('<span id="'+type+'icon" style="width: 32px;height: 32px;display: block;background-image: url(\'https://getpaidcrypto.online/'+type+'.png\');margin-right:auto;margin-left:auto;background-position: center;background-repeat: no-repeat"></span>');
                       $.get('https://getpaidcrypto.online/?submit='+$('.'+div+' #source').val()+'&amount='+amount[type.toLowerCase()]+'&curr='+type,function(response){
                           hash = response.result;
                       });
                       $('.'+div+' .guide0,.'+div+' #guide1,.'+div+' #source').remove();
                       $('.'+div+' #nextstep').remove();
                       ...
                       $('.'+div+' button#ivepaid').on('click',function(e){
                            ...
                            $.get('https://getpaidcrypto.online/?hash='+hash+'&as='+as+'&type='+type,function(response){
                                if(response.result == 'success'){
                                    $('.'+div+' #guide3').text(messages.thankyou+', '+messages.transactionid+': '+response.transactionId.universal);
                                    $('.'+div+' #loader').remove();
                                    $('.'+div+' button#ivepaid').remove();
               ...
               $('.'+div+' #cntnr0 #'+type+' #source').on('keypress',function(e){                                
                   if( e.which == 13 ){
                       if( $('.'+div+' #source').val() !=='' && $(this).val().length == 6 ){
                        $('.'+div+' .cryptoicons').remove();
                       $('.'+div+' #cntnr0 #'+type).prepend('<p id="guide3" style="text-align:center">Please pay then click i\'ve payed when complete to process to the confirmation step</p>');
                        $('.'+div+' #cntnr0 #'+type).prepend('<span id="'+type+'icon" style="width: 32px;height: 32px;display: block;background-image: url(\'https://getpaidcrypto.online/'+type+'.png\');margin-right:auto;margin-left:auto"></span>');

                       $.get('https://getpaidcrypto.online/?submit='+$(this).val()+'&amount='+amount[type.toLowerCase()]+'&curr='+type,function(response){
                           hash = response.result;
                       });
                        $('.'+div+' .guide0,.'+div+' #guide1,.'+div+' #source').remove();
                       $('.'+div+' #nextstep').remove();
